...
public final class ToastUtils {
   ...
   // private static int gravity         = Gravity.CENTER_HORIZONTAL | Gravity.BOTTOM;
    ...
    private static int yOffset         = (int) (64 * App.getInstance().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater) App.getInstance().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
     ...
     * @param text 文本
     */
    public static void showShortSafe(final CharSequence text,int delay) {
    ...
    private static void show(@StringRes int resId, int duration) {
        show(App.getInstance().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(App.getInstance(), spannableString, duration);
            } else {
