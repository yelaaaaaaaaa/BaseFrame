package com.example.baseframe.utils.wifi;
...
import com.example.baseframe.utils.AppUtils;
import com.example.baseframe.utils.ConvertUtils;
...
public final class WifiUtils {
    ...
    public boolean toggleWifiEnabled() {
        try {
            ...
            ALog.eTag(TAG, e, "toggleWifiEnabled");
        }
        return false;
    }
    ...
    public static String isConnectAphot() {
        try {
            ...
            ALog.eTag(TAG, e, "isConnectAphot");
        }
        return null;
    }
        ...
        try {
            ...
            if (isStatic && !TextUtils.isEmpty(ip)) {
                ...
                if (staticWifiConfig == null) {
                    ...
                    // =
                    ALog.dTag(TAG, "属于正常方式连接 (DHCP)");
                } else {
                    // 设置静态信息
                    connWifiConfig = staticWifiConfig;
                    // =
                    ALog.dTag(TAG, "属于静态 IP 方式连接");
                }
            } else {
                // 创建正常的配置信息
                connWifiConfig = createWifiConfig(ssid, pwd, type, true);
            ...
            if (preWifiConfig != null) {
                ...
                // 打印结果
                ALog.dTag(TAG, "删除旧的配置信息 - " + preWifiConfig.SSID + ", isRemove: " + isRemove);
                // 保存配置
            ...
            if (nId != -1) {
                try {
                    ...
                    // 打印断开连接结果
                    ALog.dTag(TAG, "isDisConnect: " + isDisConnect + ", isDisable: " + isDisable);
                } catch (Exception e) {
                    ALog.eTag(TAG, e, "quickConnWifi - 关闭连接出错: " + nId);
                }
                // 开始连接
                ...
                // 打印结果
                ALog.dTag(TAG, "addNetwork(enableNetwork) - result: " + isResult);
            } else {
                // 尝试不带引号 SSID 连接
            ...
            // 连接的 networkId
            ALog.dTag(TAG, "连接的 SSID - networkId: " + nId);
            // 返回连接的信息
            return connWifiConfig;
        } catch (Exception e) {
            ALog.eTag(TAG, "quickConnWifi", e);
        }
        return null;
    }
...
//					wifiConfig.allowedProtocols.set(WifiConfiguration.Protocol.WPA);
            ...
            ALog.eTag(TAG, e, "createWifiConfig");
        }
        return null;
    }
    ...
    public boolean removeWifiConfig(final WifiConfiguration wifiConfig) {
        ...
        try {
            ...
            ALog.eTag(TAG, "removeWifiConfig", e);
        }
        return false;
    }
    ...
    public boolean disconnectWifi(final int networkId) {
        try {
            ...
            ALog.eTag(TAG, "disconnectWifi", e);
        }
        return false;
    }
    ...
    private WifiConfiguration setStaticWifiConfig(final WifiConfiguration wifiConfig, final String ip) {
        ...
        if (ip != null) {
            try {
                ...
                ALog.eTag(TAG, e, "setStaticWifiConfig");
                return null;
            }
        }
